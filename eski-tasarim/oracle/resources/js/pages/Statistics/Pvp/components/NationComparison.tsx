  ...
  if (totalKills === 0) {
    return (
      <div className="bg-ko-card border border-ko-border-primary rounded-xl p-6 h-full">
        <CardHeader
          title={t('plugins.game.statistics_page.pvp.nation_comparison.title')}
          subtitle={t('plugins.game.statistics_page.pvp.nation_comparison.subtitle')}
          icon="ti ti-trophy"
    ...
    <div className="bg-ko-card border border-ko-border-primary rounded-xl p-6 h-full">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className={`relative p-4 rounded-xl border-2 ${karusLeading ? 'border-red-500 bg-red-500/10' : 'border-ko-border-primary bg-ko-widget-bg/50'}`}>
          ...
          <div className="space-y-2">
            <div>
              <div className="text-xs text-ko-text-muted">{t('plugins.game.statistics_page.pvp.nation_comparison.total_kills')}</div>
              <div className="text-xl font-bold text-red-400">{comparison.karus.totalKills}</div>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <div className="text-ko-text-muted">{t('plugins.game.statistics_page.pvp.nation_comparison.warriors')}</div>
                <div className="font-semibold text-ko-text-primary">{comparison.karus.uniqueWarriors}</div>
              </div>
              <div>
                <div className="text-ko-text-muted">{t('plugins.game.statistics_page.pvp.nation_comparison.avg_kd')}</div>
                <div className="font-semibold text-ko-text-primary">{comparison.karus.avgKd}</div>
              </div>
            </div>
            {comparison.karus.topKiller && (
              <div className="pt-2 border-t border-ko-border-primary/50">
                <div className="text-[10px] text-ko-text-muted uppercase">{t('plugins.game.statistics_page.pvp.nation_comparison.top_killer')}</div>
                <div className="text-sm font-semibold text-red-300 truncate">{comparison.karus.topKiller}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className={`relative p-4 rounded-xl border-2 ${!karusLeading ? 'border-blue-500 bg-blue-500/10' : 'border-ko-border-primary bg-ko-widget-bg/50'}`}>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <div className="text-ko-text-muted">{t('plugins.game.statistics_page.pvp.nation_comparison.warriors')}</div>
                <div className="font-semibold text-ko-text-primary">{comparison.human.uniqueWarriors}</div>
              </div>
              <div>
                <div className="text-ko-text-muted">{t('plugins.game.statistics_page.pvp.nation_comparison.avg_kd')}</div>
                <div className="font-semibold text-ko-text-primary">{comparison.human.avgKd}</div>
              </div>
            </div>
            {comparison.human.topKiller && (
              <div className="pt-2 border-t border-ko-border-primary/50">
                <div className="text-[10px] text-ko-text-muted uppercase">{t('plugins.game.statistics_page.pvp.nation_comparison.top_killer')}</div>
                <div className="text-sm font-semibold text-blue-300 truncate">{comparison.human.topKiller}</div>
              </div>
            )}
          </div>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2 text-sm">
          ...
          <span className="text-ko-text-muted">{t('plugins.game.statistics_page.pvp.nation_comparison.kill_share')}</span>
