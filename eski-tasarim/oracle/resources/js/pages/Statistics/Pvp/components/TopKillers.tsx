...
import { getKdRarity } from './types'

const ITEMS_PER_PAGE = 5

export function TopKillers() {
  const { t } = useTranslation()
  const { props } = usePage()
  const { data, periods } = usePvp()
  ...
  const getFilterLabel = () => {
    const parts = []
    if (filters.nation > 0) {
      parts.push(filters.nation === 1 ? 'Karus' : 'El Morad')
    }
    if (filters.job > 0) {
  ...
  if (rankings.length === 0) {
    return (
      <div className="bg-ko-card border border-ko-border-primary rounded-xl p-6">
        <CardHeader
          title={t('plugins.game.statistics_page.pvp.rankings_title')}
          subtitle={t('plugins.game.statistics_page.pvp.rankings_subtitle')}
          icon="ti ti-swords"
          iconGradient="from-red-500 to-orange-500"
        />
        <EmptyState icon="ti ti-swords" height={150} />
      </div>
    )
  }
    ...
    <div className="bg-ko-card border border-ko-border-primary rounded-xl p-6">
      ...
      <div className="space-y-2">
        {visibleRankings.map((player, index) => {
          const kdRarity = getKdRarity(player.kd)
          return (
            <div
              key={player.userId}
              ...
              <div className="flex items-center gap-4 flex-shrink-0">
                ...
                <div className="text-center">
                  ...
                  <div className="text-[10px] text-ko-text-muted uppercase">
                    {t('plugins.game.statistics_page.pvp.headers.kills')}
                  </div>
                </div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-[10px] text-ko-text-muted uppercase">
                    {t('plugins.game.statistics_page.pvp.headers.deaths')}
                  </div>
                </div>
                ...
                <div className="text-center min-w-[50px]">
                  <div
                    ...
                    style={{ backgroundColor: kdRarity.color, color: kdRarity.textColor }}
                  >
                    {player.kd.toFixed(2)}
                  </div>
                  <div className="text-[10px] text-ko-text-muted uppercase">K/D</div>
                </div>
              </div>
            </div>
          )
      ...
      {hasMore && (
        <div className="mt-4 pt-4 border-t border-ko-border-primary">
          ...
          >
            ...
            {Math.min(ITEMS_PER_PAGE, remainingCount)} {t('plugins.game.statistics_page.pvp.load_more_label')}
          </Button>
        </div>
      )}
    </div>
