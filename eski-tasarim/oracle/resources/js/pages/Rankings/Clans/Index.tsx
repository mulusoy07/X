...
export default function ClansRankings() {
  ...
  const { rankings, filters, nations, sortOptions, hasMore, total } = data
  const page = filters.page

  const navigate = useCallback((query) => {
    setIsRefreshing(true)
    router.get(route('public.rankings.clans'), query, {
      preserveState: true,
      preserveScroll: true,
      replace: true,
  ...
  return (
    <>
      <Head title={t('rankings.clans.title')} />

      <div className="bg-gradient-to-br from-ko-main via-ko-card to-ko-main min-h-screen">
        <div className="container mx-auto px-4 py-6">
          <PageHeader
            title={t('rankings.clans.title')}
            subtitle={t('rankings.clans.subtitle')}
...
ClansRankings.layout = (page) => <PublicLayout children={page} />
