...
export function FilterSection({ filters, nations, jobs, sortOptions, searchRef, isRefreshing, onFilterChange, onReset }) {
    ...
    <div className="bg-ko-card-bg border border-ko-border-primary rounded-lg p-3 sm:p-4">
      <div className="flex flex-wrap items-center gap-2 sm:gap-3">
        <div className="w-full sm:flex-1 relative order-1">
          ...
          <Input
            key={filters.userName}
            ref={searchRef}
            placeholder={t('rankings.users.search')}
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') onFilterChange('userName', e.currentTarget.value)
            }}
          />
        </div>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto order-3">
          <Select
            ...
            disabled={isRefreshing}
          >
            <SelectTrigger aria-label={t('rankings.select_nation')} className="flex-1 sm:flex-none sm:w-32 bg-ko-widget-bg border-ko-border-primary text-ko-text-card-title focus:border-ko-brand-primary focus:ring-ko-brand-primary/20 hover:border-ko-brand-primary/50 transition-colors h-9">
          ...
          <Select
            ...
            onValueChange={(value) => onFilterChange('job', parseInt(value))}
            disabled={isRefreshing}
          >
            <SelectTrigger aria-label={t('rankings.users.select_job')} className="flex-1 sm:flex-none sm:w-32 bg-ko-widget-bg border-ko-border-primary text-ko-text-card-title focus:border-ko-brand-primary focus:ring-ko-brand-primary/20 hover:border-ko-brand-primary/50 transition-colors h-9">
              <SelectValue placeholder={t('rankings.users.select_job')} />
            </SelectTrigger>
            <SelectContent className="bg-ko-card-bg border-ko-border-primary shadow-lg">
              {jobs.map((name, index) => (
                <SelectItem key={index} value={index.toString()} className="text-ko-text-card-title hover:bg-ko-card-hover hover:text-ko-brand-primary focus:bg-ko-card-hover focus:text-ko-brand-primary data-[highlighted]:bg-ko-card-hover data-[highlighted]:text-ko-brand-primary data-[state=checked]:bg-ko-brand-primary/20 data-[state=checked]:text-ko-brand-primary cursor-pointer">
          ...
          <Select
            ...
            disabled={isRefreshing}
          >
            <SelectTrigger aria-label={t('rankings.sort_by')} className="flex-1 sm:flex-none sm:w-32 bg-ko-widget-bg border-ko-border-primary text-ko-text-card-title focus:border-ko-brand-primary focus:ring-ko-brand-primary/20 hover:border-ko-brand-primary/50 transition-colors h-9">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto order-4">
          <Button
            ...
            onClick={() => {
              if (searchRef?.current) onFilterChange('userName', searchRef.current.value)
            }}
          >
            {isRefreshing
