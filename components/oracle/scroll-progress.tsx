...
export function ScrollProgress() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Check if near bottom (90% or more)
      setIsAtBottom(percent >= 90)
    }
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title={isAtBottom ? "Yukari git" : "Asagi git"}
      >
        {/* SVG Progress Circle */}
        <svg
        ...
        {/* Icon - changes direction based on scroll position */}
        {isAtBottom ? (
          <IconArrowUp className="w-4 h-4 text-cream-dim group-hover:text-gold-400 transition-colors relative z-10" />
        ) : (
          <IconArrowDown className="w-4 h-4 text-cream-dim group-hover:text-gold-400 transition-colors relative z-10" />
        )}
